...
@section('content')
	...
	<main class="w-full min-w-0 p-4 sm:p-6 lg:p-8">
		...
		<div
			class="relative bg-white rounded-2xl shadow-md border {{ $booking->status === 'Completed' ? 'border-emerald-200' : 'border-slate-200' }} mb-6 print:hidden overflow-hidden"
			id="bookingForm">
			...
			<div class="p-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
				<div class="md:col-span-5 border-b md:border-b-0 md:border-r border-slate-100 pb-4 md:pb-0 md:pr-4">
					...
					@role('officer')
						<div class="mt-4 flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-2.5">
							<div class="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center shrink-0">
								<i class="fa-solid fa-user text-emerald-600 text-sm"></i>
							</div>

							<div class="min-w-0">
								<p class="text-[9px] font-bold uppercase tracking-wider text-emerald-600"> Farmer
								</p>
				...
				<div class="md:col-span-7 grid grid-cols-2 sm:grid-cols-4 gap-4 text-left">
					<div>
						<p class="text-[10px] font-bold tracking-wider text-slate-400 uppercase mb-1"> <i
								class="fa-regular fa-calendar text-slate-400 mr-1"></i> Start Date
						</p>
					...
					<div>
						...
						<p class="text-[10px] font-bold tracking-wider text-slate-400 uppercase mb-1"> <i
								class="fa-regular fa-calendar-check text-slate-400 mr-1"></i> End Date
						</p>
					...
					<div>
						<p class="text-[10px] font-bold tracking-wider text-slate-400 uppercase mb-1"> <i
								class="fa-regular fa-clock text-slate-400 mr-1"></i> Duration </p>
					...
					<div>
						<p class="text-[10px] font-bold tracking-wider text-slate-400 uppercase mb-1"> <i
								class="fa-solid fa-hourglass-half text-slate-400 mr-1"></i> Total Hours
						</p>
			...
			<div class="{{ $statusConfig['summaryBg'] }} px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
				<div class="flex items-center gap-3 w-full">
					<div
						...
						<i
							class="fa-solid {{ $booking->status === 'Completed' ? 'fa-circle-check' : ($booking->status === 'Declined' ? 'fa-triangle-exclamation' : 'fa-receipt') }}"></i>i
					</div>
				...
				@if ($booking->status !== 'Declined')
					<div class="flex items-center gap-3 w-full sm:w-auto justify-end shrink-0">
						...
						<div class="{{ $statusConfig['amountBg'] }} text-white px-4 py-2 rounded-xl flex items-center gap-1 shadow-sm">
							...
							@role('officer')
								...
								@elseif($booking->status === 'Approved')
									<x-confirm-modal title="Complete Booking" :message="'Are you sure to complete this Booking?'" confirmText="Complete"
										confirmClass="bg-green-600 hover:bg-green-700 text-white" icon="shield-alert" :action="route('farmers.completeBooking', $booking->id)" method="PUT"
										:data='"<input type=\"hidden\" name=\"total_hours\" id=\"total_hours\"> <input type=\"hidden\" name=\"total_cost\" id=\"total_cost\">"'>
