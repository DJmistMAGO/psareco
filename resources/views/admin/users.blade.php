...
@section('content')
    <main class="w-full min-w-0 p-4 sm:p-6 lg:p-8">
            ...
            <div @click.outside="open = false"
                ...
                <form action="{{ route('user-management.adduser') }}" method="POST" class="p-5">
                    ...
                    <div class="grid grid-cols-1 gap-y-4">
                        ...
                        <div>
                            <label for="contact_number" class="block text-xs font-semibold text-slate-600 mb-1">Contact
                                Number</label>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <input type="password" id="password" name="password" x-ref="passwordInput"
                            ...
                            <p class="mt-1 text-[10px] text-slate-400">User will be required to change this on first login.
                            </p>
                            @error('password')
                                <p class="mt-1 text-red-500 text-[11px] font-medium flex items-center gap-1">
                                    <i class="fa-solid fa-circle-info text-[10px]"></i> {{ $message }}
        ...
        <div x-data="{
            ...
            <div @click.outside="open = false"
                ...
                <div class="flex items-center justify-between p-5 border-b border-slate-100">
                    <h3 class="font-bold text-slate-800 text-sm flex items-center gap-2">
                        <i class="fa-solid"
                            :class="selectedUser.is_admin ? 'fa-shield-halved text-amber-600' : (selectedUser
                                .role === 'officer' ?
                ...
                <form :action="formAction" method="POST" class="p-5">
                    ...
                    <div class="mb-5">
                        ...
                        <div class="grid grid-cols-2 gap-2 rounded-xl bg-slate-50 border border-slate-200 p-1">
                            <button type="button" @click="selectedUser.role = 'officer'"
                                :disabled="selectedUser.is_admin"
                                :class="selectedUser.role === 'officer' ?
                                    'bg-white text-sky-700 shadow-sm border border-slate-200' :
                                    'text-slate-500 hover:text-slate-700'"
                            ...
                            <button type="button" @click="selectedUser.role = 'farmer'" :disabled="selectedUser.is_admin"
                                :class="selectedUser.role === 'farmer' ?
                                    'bg-white text-emerald-700 shadow-sm border border-slate-200' :
                                    'text-slate-500 hover:text-slate-700'"
                                class="flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-semibold transition disabled:opacity-60 disabled:cursor-not-allowed">
                                <i class="fa-solid fa-seedling text-[10px]"></i> Farmer
                    ...
                    <div class="grid grid-cols-1 gap-y-4">
                        ...
                        <div>
                            ...
                            <button type="button"
                                ...
                                :class="selectedUser.status === 'active' ? 'bg-emerald-50 border-emerald-200' :
                            ...
                            <p class="mt-1 text-[11px] text-slate-400"
                                x-show="selectedUser.is_self && !selectedUser.is_admin">You
                        ...
                        <div x-show="selectedUser.role === 'officer'">
                            ...
                            <input type="text" name="position" x-model="selectedUser.position"
                                :disabled="selectedUser.is_admin"
                                class="w-full px-3 py-2 bg-slate-50 border rounded-xl text-xs text-slate-800 focus:outline-none transition border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:bg-white disabled:opacity-60 disabled:cursor-not-allowed">
                        </div>
                    </div>
                    ...
                    <template x-if="selectedUser.is_admin">
                        <div class="mt-5 rounded-2xl border border-amber-100 bg-amber-50 p-4 flex items-center gap-3">
                            <div
                                class="w-9 h-9 rounded-xl flex items-center justify-center bg-amber-100 text-amber-600 shrink-0">
                                <i class="fa-solid fa-shield-halved"></i>
                            </div>
                            <div>
                                <p class="text-xs font-semibold text-amber-700">Protected Account</p>
                                <p class="text-[11px] text-amber-600 mt-0.5">Administrator accounts can't be edited from
                                    User Management.
                                </p>
                            </div>
                        </div>
        ...
        {{-- Alpine State Container for Tabs, Search, and Role Filter --}}
        <div x-data="{
            activeTab: 'active',
            search: '',
            roleFilter: ''
            ...
            <div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    <button type="button" @click="activeTab = 'active'"
                        :class="activeTab === 'active' ? 'bg-emerald-50 text-emerald-800' : 'text-slate-500 hover:bg-slate-50'"
                        class="px-3 py-1.5 text-xs font-semibold rounded-xl transition">
                        Active Users <span
                            class="bg-emerald-500 text-white text-[10px] px-2 py-0.2 rounded-full font-bold">
                            {{ $activeUsers->count() }}</span>
                    </button>
                    <button type="button" @click="activeTab = 'inactive'"
                        :class="activeTab === 'inactive' ? 'bg-red-50 text-red-800' : 'text-slate-500 hover:bg-slate-50'"
                        class="px-3 py-1.5 text-xs font-semibold rounded-xl transition flex items-center gap-1.5">
                        Deactivated Users
                        <span
                ...
                {{-- Search & Role Filters --}}
                <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto print:hidden">

                    {{-- Role Filter --}}
                    <select x-model="roleFilter"
                        class="w-full sm:w-auto px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white transition">
            ...
            {{-- Active Users Table --}}
            <div x-show="activeTab === 'active'" class="w-full overflow-x-auto">
                <table class="w-full text-left border-collapse text-xs">
                    ...
                    <tbody class="divide-y divide-slate-100 text-slate-700">
                        @forelse($activeUsers as $user)
                            <tr x-show="
                                (!roleFilter || @js($user['roles']).map(r => r.toLowerCase()).includes(roleFilter)) &&
                                (!search || @js(implode(' ', [$user['name'], $user['email'], implode(' ', $user['roles'])])).toLowerCase().includes(search.toLowerCase()))
                            " class="group hover:bg-emerald-50/40 transition-colors duration-150">
                                ...
                                <td class="py-3.5 px-4">
                                    <span
                                        class="inline-flex items-center gap-2 text-[11px] font-semibold text-emerald-700">
                                        <span class="relative flex h-2 w-2">
                                            <span
                                ...
                                <td class="py-3.5 px-4 text-center print:hidden">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        @if (in_array('admin', $user['roles'], true))
                                            <span
                                                class="inline-flex items-center gap-1.5 text-[11px] italic text-slate-400">
                                        ...
                                        @elseif(auth()->id() !== $user['id'])
                                            <x-confirm-modal title="Deactivate User"
                                                ...
                                                confirm-text="Deactivate" cancel-text="Cancel"
                                                confirm-class="bg-red-600 hover:bg-red-700" :action="route('user-management.deactivateUser', $user['id'])"
                                                method="POST">
                                                <button type="button"
            ...
            <div x-show="activeTab === 'inactive'" x-cloak class="w-full overflow-x-auto">
                <table class="w-full text-left border-collapse text-xs">
                    <thead>
                        <tr
                            class="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase text-[10px] tracking-wider font-semibold">
