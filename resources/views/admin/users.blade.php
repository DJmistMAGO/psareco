    ...
    generatePassword() {
			...
			<div @click.outside="open = false"
				...
				<form action="{{ route('user-management.adduser') }}" method="POST" class="p-5">
					...
					<div class="grid grid-cols-1 gap-y-4">
						...
						<div>
							<label for="password" class="block text-xs font-semibold text-slate-600 mb-1"> Temporary Password <span
									class="text-red-500">*</span> </label>
							<div class="flex gap-2">
								<input type="password" id="password" name="password" x-ref="passwordInput" placeholder="••••••••" required
									minlength="8"
				...
				<form :action="formAction" method="POST" class="p-5">
					...
					<div class="grid grid-cols-1 gap-y-4">
						...
						<div>
							...
							<p class="mt-1 text-[11px] text-slate-400" x-show="selectedUser.is_self && !selectedUser.is_admin">You can't
								change your own status here.</p>
						</div>
					...
					<div class="flex justify-end gap-2 mt-5 pt-4 border-t border-slate-100">
						<button type="button" @click="open = false"
							class="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 hover:bg-slate-50 transition"> Close </button>
						<button type="submit" x-show="!selectedUser.is_admin"
							class="inline-flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white font-semibold text-xs py-2 px-4 rounded-xl shadow-sm transition-all cursor-pointer">
							<i class="fa-regular fa-floppy-disk text-[11px]"></i> Save Changes </button>
					</div>
				</form>
			</div>
		</div>
		...
		<div class="bg-white rounded-2xl shadow-sm border border-slate-100/80 overflow-hidden">
			<div class="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
				...
				<div class="flex items-center gap-2">
					<button id="activeTabBtn" onclick="switchTab('active')"
						class="px-3 py-1.5 text-xs font-semibold rounded-xl bg-emerald-50 text-emerald-800 transition flex items-center gap-1.5">
						Active Users
						<span
							class="bg-emerald-500 text-white text-[10px] px-2 py-0.2 rounded-full font-bold">{{ $activeUsers->count() }}</span>
					</button>
			...
			<div id="activeSection" class="w-full overflow-x-auto">
				<div class="px-4 pt-4">
					<div class="flex items-center gap-1 rounded-xl bg-slate-50 border border-slate-200 p-1 w-fit">
						<button type="button" id="activeAllTabBtn" onclick="switchRoleTab('active', 'all')"
							class="px-3 py-1.5 rounded-lg bg-white text-emerald-700 shadow-sm border border-slate-200 text-[11px] font-semibold transition">
							All
							<span class="ml-1 text-[10px] text-slate-400">{{ $activeUsers->count() }}</span>
						</button>

						<button type="button" id="activeFarmerTabBtn" onclick="switchRoleTab('active', 'farmer')"
							class="px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-700 text-[11px] font-semibold transition">
							Farmers
							<span class="ml-1 text-[10px] text-slate-400">{{ $activeFarmers->count() }}</span>
						</button>

						<button type="button" id="activeOfficerTabBtn" onclick="switchRoleTab('active', 'officer')"
							class="px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-700 text-[11px] font-semibold transition">
							Officers
							<span class="ml-1 text-[10px] text-slate-400">{{ $activeOfficers->count() }}</span>
						</button>
					</div>
				</div>
				...
				<table class="w-full text-left border-collapse text-xs mt-3">
					...
					<tbody id="activeUsersTable" class="divide-y divide-slate-100 text-slate-700">
						@forelse($activeUsers as $user)
							<tr data-role="{{ $user['roles'][0] ?? 'farmer' }}"
								class="group hover:bg-emerald-50/40 transition-colors duration-150">
								...
								<td class="py-3.5 px-4">
									<div class="flex flex-wrap gap-1.5">
										@foreach ($user['roles'] as $role)
											...
											<span
												class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold {{ $badgeClass }}">
												<i class="fa-solid {{ $roleIcon }} text-[9px]"></i>
												{{ ucfirst($role) }}
											</span>
										@endforeach
									</div>
								...
								<td class="py-3.5 px-4 text-center print:hidden">
									<div class="flex items-center justify-center gap-2">
										<button type="button"
											@click="$dispatch('open-view-user-modal', { user: { id: {{ $user['id'] }}, name: @js($user['name']), email: @js($user['email']), contact_number: @js($user['contact_number'] ?? ''), address: @js($user['address'] ?? ''), position: @js($user['position'] ?? ''), role: @js($user['roles'][0] ?? 'farmer'), status: 'active', is_self: {{ auth()->id() === $user['id'] ? 'true' : 'false' }}, is_admin: false, created_at: @js(optional($user['created_at'])->format('M d, Y')) } })"
										...
										<x-confirm-modal title="Reset Password"
											...
											<button type="button"
												class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600 shadow-sm transition-all hover:bg-slate-50 hover:border-slate-300">
												<i class="fa-solid fa-key text-[10px]"></i> Reset Password </button>
										</x-confirm-modal>
										...
										@if (auth()->id() !== $user['id'])
											<x-confirm-modal title="Deactivate User"
												...
												<button type="button"
													...
													<i class="fa-solid fa-user-slash text-[10px]"></i> Deactivate </button>
											</x-confirm-modal>
										@else
			...
			<div id="inactiveSection" class="w-full overflow-x-auto hidden">
				<div class="px-4 pt-4">
					<div class="flex items-center gap-1 rounded-xl bg-slate-50 border border-slate-200 p-1 w-fit">
						<button type="button" id="inactiveAllTabBtn" onclick="switchRoleTab('inactive', 'all')"
							class="px-3 py-1.5 rounded-lg bg-white text-red-700 shadow-sm border border-slate-200 text-[11px] font-semibold transition">
							All
							<span class="ml-1 text-[10px] text-slate-400">{{ $inactiveUsers->count() }}</span>
						</button>

						<button type="button" id="inactiveFarmerTabBtn" onclick="switchRoleTab('inactive', 'farmer')"
							class="px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-700 text-[11px] font-semibold transition">
							Farmers
							<span class="ml-1 text-[10px] text-slate-400">{{ $inactiveFarmers->count() }}</span>
						</button>

						<button type="button" id="inactiveOfficerTabBtn" onclick="switchRoleTab('inactive', 'officer')"
							class="px-3 py-1.5 rounded-lg text-slate-500 hover:text-slate-700 text-[11px] font-semibold transition">
							Officers
							<span class="ml-1 text-[10px] text-slate-400">{{ $inactiveOfficers->count() }}</span>
						</button>
					</div>
				</div>
				...
				<table class="w-full text-left border-collapse text-xs mt-3">
					...
					<tbody id="inactiveUsersTable" class="divide-y divide-slate-100 text-slate-700">

						@forelse($inactiveUsers as $user)
							<tr data-role="{{ $user['roles'][0] ?? 'farmer' }}"
								class="group hover:bg-red-50/30 transition-colors duration-150">
								<td class="py-3.5 px-4">
									<span class="font-mono text-[11px] text-slate-400">
										#{{ $user['id'] }}
									</span>
								...
								<td class="py-3.5 px-4">
									@php
										$inactiveRole = $user['roles'][0] ?? 'farmer';
										$inactiveRoleClass =
										    $inactiveRole === 'officer'
										        ? 'border-sky-200 bg-sky-50 text-sky-700'
										        : 'border-slate-200 bg-slate-50 text-slate-600';
										$inactiveRoleIcon = $inactiveRole === 'officer' ? 'fa-user-tie' : 'fa-seedling';
									@endphp

									<span
										class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold {{ $inactiveRoleClass }}">
										<i class="fa-solid {{ $inactiveRoleIcon }} text-[9px]"></i>
										{{ ucfirst($inactiveRole) }}
									</span>
								</td>
								...
								<td class="py-3.5 px-4 text-center print:hidden">
									<div class="flex items-center justify-center gap-2">
										...
										<x-confirm-modal title="Reactivate User"
											...
											confirm-text="Reactivate" cancel-text="Cancel" confirm-class="bg-emerald-600 hover:bg-emerald-700"
											:action="route('user-management.reactivateUser', $user['id'])" method="POST">
											<button type="button"
										...
										<x-confirm-modal title="Delete User Permanently"
											...
											confirm-text="Delete Permanently" cancel-text="Cancel" confirm-class="bg-red-600 hover:bg-red-700"
											:action="route('user-management.deleteUser', $user['id'])" method="POST">
											<button type="button"
												class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-red-600 shadow-sm transition-all hover:bg-red-50 hover:border-red-300">
...
<script>
	...
	function switchTab(status) {
		...
		document.getElementById('activeSection').classList.toggle('hidden', status !== 'active');
		...
		activeTabBtn.classList.toggle('bg-emerald-50', status === 'active');
		activeTabBtn.classList.toggle('text-emerald-800', status === 'active');
		activeTabBtn.classList.toggle('text-slate-500', status !== 'active');

		inactiveTabBtn.classList.toggle('bg-red-50', status === 'inactive');
		inactiveTabBtn.classList.toggle('text-red-700', status === 'inactive');
		inactiveTabBtn.classList.toggle('text-slate-500', status !== 'inactive');

		filterUsers();
	}

	function switchRoleTab(status, role) {
		currentRoleTab[status] = role;

		const prefix = status === 'active' ? 'active' : 'inactive';

		['all', 'farmer', 'officer'].forEach(tab => {
			const tabName = tab.charAt(0).toUpperCase() + tab.slice(1);
			const button = document.getElementById(`${prefix}${tabName}TabBtn`);
			...
			if (isActive && status === 'active') {
				button.classList.add('text-emerald-700');
				button.classList.remove('text-red-700', 'text-slate-500');
			} else if (isActive && status === 'inactive') {
				button.classList.add('text-red-700');
				button.classList.remove('text-emerald-700', 'text-slate-500');
			} else {
	...
	function filterUsers() {
		...
		const tableId = currentStatusTab === 'active' ?
			'activeUsersTable' :
			'inactiveUsersTable';
		...
		const role = currentRoleTab[currentStatusTab];

		table.querySelectorAll('tr[data-role]').forEach(row => {
			const matchesRole = role === 'all' || row.dataset.role === role;
			const matchesSearch = row.textContent.toLowerCase().includes(search);

			row.classList.toggle('hidden', !(matchesRole && matchesSearch));
		});
	}

	document.addEventListener('DOMContentLoaded', function() {
		switchTab('active');
		switchRoleTab('active', 'all');
		switchRoleTab('inactive', 'all');
	});
</script>
