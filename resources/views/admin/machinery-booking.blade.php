			...
			<div class="p-5 border-b border-slate-100">
				...
				<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
					...
					<form action="{{ url()->current() }}" method="GET" class="relative w-full sm:w-64">
						...
						<input type="text" name="search" value="{{ request('search') }}" placeholder="Search farmer or machinery..."
				...
				<div class="flex items-center gap-1.5 mt-5 overflow-x-auto pb-1">
					...
					{{-- Declined / Cancelled --}}
					<a
						href="{{ url()->current() }}?status=declined{{ request('search') ? '&search=' . urlencode(request('search')) : '' }}"
							...
							<tr class="hover:bg-slate-50/60 transition">
								<td class="px-5 py-4">
									...
									<div class="flex items-center gap-3">
										<div class="h-9 w-9 shrink-0 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center">
									...
									@if ($booking->status === 'Pending')
										<div x-data="{ declineModalOpen: false, submitting: false }" class="inline-flex items-center gap-1.5">
											...
											<x-confirm-modal title="Approve Booking" :message="'Are you sure you want to approve this booking?'" confirmText="Approve"
												...
												method="PUT" :data='"<input type=\"hidden\" name=\"status\" value=\"Approved\">"'>
											...
											<button type="button" @click="declineModalOpen = true" title="Decline Booking"
											...
											<template x-teleport="body">
												<div x-show="declineModalOpen" x-cloak class="fixed inset-0 z-50 overflow-y-auto font-sans"
													aria-labelledby="decline-modal-title-{{ $booking->id }}" role="dialog" aria-modal="true">
													...
													<div class="flex min-h-full items-center justify-center p-4 text-center">
														<div x-show="declineModalOpen" x-transition:enter="ease-out duration-300"
															...
															<form action="{{ route('officer.decline-booking', $booking->id) }}" method="POST"
																...
																<div class="bg-gradient-to-r from-red-50 via-rose-50/40 to-white px-6 py-4 border-b border-red-100">
																	<div class="flex items-center gap-3">
																		<div
																		...
																		<div>
																			<h3 class="text-base font-semibold text-slate-800" id="decline-modal-title-{{ $booking->id }}">
																				Decline Booking
																			</h3>

																			<p class="text-[11px] font-medium tracking-wide uppercase text-red-600">
																...
																<div class="px-6 py-6">
																	...
																	<div class="mt-5">
																		<label for="remarks-{{ $booking->id }}" class="mb-2 block text-sm font-semibold text-slate-700">
