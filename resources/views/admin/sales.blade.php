...
@section('content')
	<main class="w-full min-w-0 p-4 sm:p-6 lg:p-8">
		...
		<x-page-header eyebrow="PSARECO Sales" title="Sales Transactions"
			description="Manage product orders, process point-of-sale checkouts, and review sales history"
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6 print:hidden">
			<div class="lg:col-span-7 bg-white rounded-2xl shadow-sm border border-slate-100/80 p-5 flex flex-col justify-between"
				id="recordSaleCard">
				<div>
					...
					<div class="relative mb-3">
						...
						<input type="text" id="productSearchInput" onkeyup="filterProductList()"
						...
						@forelse ($products as $product)
							<div
								...
								<div>
									...
									<div class="text-[11px] text-slate-500">Stock: <span class="font-medium text-slate-700">{{ $product->quantity }}
				...
				<div class="pt-4 border-t border-slate-100 space-y-3">
					<div id="stockInfo"
						class="hidden p-2.5 rounded-xl bg-emerald-50 border border-emerald-200/60 text-xs text-emerald-900 font-medium">
					</div>
			...
			<div
				class="lg:col-span-5 bg-white rounded-2xl shadow-sm border border-slate-100/80 p-5 flex flex-col justify-between">
				<div>
					<div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
						<h3 class="font-bold text-slate-800 text-sm flex items-center gap-2">
				...
				<div class="pt-4 border-t border-slate-100 space-y-3">
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
						<div>
							<label class="block text-xs font-semibold text-slate-600 mb-1">Buyer Name <span
...
@push('scripts')
	<script>
		...
		function selectProduct(id, name, price, stock, unit) {
			...
			stockInfo.innerHTML =
				`<i class="fa-solid fa-circle-check text-emerald-600"></i> <strong class="text-slate-800">${name}</strong> &bull; Stock: ${stock} ${unit} &bull; Price: ₱${Number(price).toLocaleString()}`;
		}

		function addToCart() {
			if (!currentProductForCart) {
				showStatusModal('error', 'No Product Selected',
					'Please click a product from the list above before adding it to the cart.');
				return;
			}
			...
			if (currentProductForCart.stock < qty) {
				showStatusModal('error', 'Insufficient Stock',
					`Only ${currentProductForCart.stock} ${currentProductForCart.unit} of this product are available.`);
				return;
			}
			...
			if (existing) {
				if (existing.qty + qty > currentProductForCart.stock) {
					showStatusModal('error', 'Insufficient Stock',
						`Cannot add ${qty} more — only ${currentProductForCart.stock - existing.qty} left in stock.`);
					return;
				}
				existing.qty += qty;
			} else {
		...
		function showStatusModal(type, title, message, onOk) {
			...
			if (type === 'success') {
				...
				okBtn.className =
					'mt-5 w-full inline-flex items-center justify-center bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs py-2.5 px-4 rounded-xl shadow-sm transition';
			} else {
				...
				okBtn.className =
					'mt-5 w-full inline-flex items-center justify-center bg-red-600 hover:bg-red-700 text-white font-semibold text-xs py-2.5 px-4 rounded-xl shadow-sm transition';
			}
		...
		async function checkout() {
			...
			try {
				...
				try {
					...
					showStatusModal('error', 'Checkout Failed',
						`The server returned an unexpected response (HTTP ${response.status}). Check the console for details.`
					);
					return;
				}

				if (!response.ok) {
					showStatusModal('error', 'Checkout Failed', data.message ||
