...
@section('content')
    ...
    openEdit(item) {
		...
		<main class="w-full min-w-0 p-4 sm:p-6 lg:p-8">
			<x-dashboard-header />
			<x-page-header eyebrow="PSARECO Machinery Management" title="Machinery Management"
				description="Browse agricultural machinery available for rental" icon="fa-solid fa-tractor">
				@role('officer')
					<x-slot:actions>
						<button type="button" onclick="document.getElementById('addMachineryModal').classList.remove('hidden')"
								...
								<tbody class="divide-y divide-slate-100">
									@foreach ($machineries as $item)
										@php
											...
											$deleteRoute = Route::has('machinery.deleteMachinery')
											    ? route('machinery.deleteMachinery', $item->id)
											    : (Route::has('machinery.forceDeleteMachinery')
											        ? route('machinery.forceDeleteMachinery', $item->id)
											        : '#');
										@endphp
										<tr class="hover:bg-slate-50/70 transition-colors">
											<td class="px-5 py-4">
												<div class="flex items-center gap-3">
													<div
														class="w-9 h-9 shrink-0 rounded-lg bg-emerald-100 text-emerald-600 flex items-center justify-center overflow-hidden">
														@if ($item->image_path)
											...
											<td class="px-5 py-4">
												<span
													class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold {{ $statusClasses }}">
											...
											<td class="px-5 py-4">
												<div class="flex items-center justify-end gap-1">
													<button type="button" @click="openView(@js($item))" title="View details"
														class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-emerald-500 hover:text-emerald-700 hover:bg-emerald-50 transition">
														<i class="fa-regular fa-eye text-xs"></i>
													</button>
													<button type="button" @click="openEdit(@js($item))" title="Edit machinery"
														class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition">
														<i class="fa-regular fa-pen-to-square text-xs"></i>
													</button>
													<x-confirm-modal title="Delete Machinery" :message="'Delete ' . $item->machinery_name . '? This will move it to trash — you can restore it later.'" confirmText="Delete"
														confirmClass="bg-red-600 hover:bg-red-700 text-white" icon="shield-alert" :action="$deleteRoute" method="DELETE">
														<button type="button" title="Delete machinery"
															class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-red-400 hover:text-red-600 hover:bg-red-50 transition">
							...
							@foreach ($machineries as $item)
								...
								<div class="p-4">
									<div class="flex items-start justify-between gap-3">
										...
										<span
											class="shrink-0 inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold {{ $statusClasses }}">
											{{ $item->status }}
										</span>
									</div>
					...
					<section class="bg-white border border-slate-100 rounded-2xl shadow-sm p-4 mb-6">
						<div class="flex flex-col lg:flex-row gap-3">
							<div class="relative flex-1">
								...
								<input type="text" x-model="search" placeholder="Search machinery, model, serial..."
									class="w-full pl-11 pr-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
							</div>
							...
							<button type="button" @click="search = ''; status = 'All'; sort = 'default'"
										...
										<div class="grid grid-cols-2 gap-3 mt-4">
											<div class="rounded-xl bg-slate-50 border border-slate-100 p-3">
												...
												<p class="mt-1 text-sm font-bold text-emerald-700">
													₱<span
														x-text="Number(item.price || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })"></span>
												</p>
											</div>
											<div class="rounded-xl bg-slate-50 border border-slate-100 p-3">
		...
		@role('officer')
			<div x-show="showView" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog">
				...
				<div class="relative min-h-screen flex items-center justify-center p-4">
					<div class="relative w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden" x-show="showView"
						x-transition>
							...
							<div>
								...
								<div class="p-6 grid grid-cols-1 sm:grid-cols-[200px_1fr] gap-6">
									<div>
										...
										<div
											class="w-full aspect-square rounded-2xl bg-slate-50 border border-slate-200 flex flex-col items-center justify-center overflow-hidden">
										...
										<div class="mb-6">
											...
											<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
												<div class="sm:col-span-2">
													...
													<div class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700"
														x-text="selected.machinery_name"></div>
												</div>
												<div>
													<label class="block text-xs font-semibold text-slate-600 mb-1.5">Model</label>
													<div class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700"
														x-text="selected.model || 'N/A'"></div>
												</div>
												<div>
													<label class="block text-xs font-semibold text-slate-600 mb-1.5">Serial Number</label>
													<div
														class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-mono text-slate-700"
														x-text="selected.serial_number || 'N/A'"></div>
												</div>
											</div>
										</div>
										<div>
											...
											<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
												<div>
													...
													<div
														class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-bold text-slate-700">
												...
												<div>
													...
													<div
														class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm font-bold text-emerald-700"
														x-text="selected.status"></div>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
									...
									<button type="button" @click="showView = false; openEdit(selected)"
										class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition">
			...
			<div x-show="showEdit" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog"
				x-data="{ editImagePreview: null }">
				<div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm" @click="showEdit = false"></div>
				<div class="relative min-h-screen flex items-center justify-center p-4">
					<div class="relative w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden" x-show="showEdit"
						x-transition>
						<div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
							<div class="flex items-center gap-3">
						...
						<form method="POST" :action="'{{ route('machinery.update', ['id' => ':id']) }}'.replace(':id', editForm.id)"
							...
							<div class="p-6 grid grid-cols-1 sm:grid-cols-[200px_1fr] gap-6">
								<div>
									...
									<label for="edit_image_path" class="cursor-pointer group block">
										<div
											class="w-full aspect-square rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 group-hover:border-emerald-400 flex flex-col items-center justify-center overflow-hidden relative">
											<img x-show="editImagePreview || editForm.image_path"
											...
											<template x-if="!editImagePreview && !editForm.image_path">
												<div class="flex flex-col items-center">
													...
													<span class="text-[11px] font-semibold text-slate-400 group-hover:text-emerald-500 px-4 text-center">Click
                        ...
                        ">
								...
								<div>
									<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
										<div class="sm:col-span-2">
											...
											<input type="text" name="machinery_name" x-model="editForm.machinery_name" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Model</label>
											<input type="text" name="model" x-model="editForm.model" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Serial Number</label>
											<input type="text" name="serial_number" x-model="editForm.serial_number" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Rent / Hour (₱)</label>
											<input type="number" step="0.01" name="price" x-model="editForm.price" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Status</label>
							...
							<div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
								<button type="button" @click="showEdit = false"
									class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 text-xs font-semibold hover:bg-slate-100 transition">Cancel</button>
								<button type="submit"
									class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition">Save
									Changes</button>
							</div>
						</form>
					</div>
			...
			<div id="addMachineryModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog">
				<div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm"
					onclick="document.getElementById('addMachineryModal').classList.add('hidden')"></div>
				<div class="relative min-h-screen flex items-center justify-center p-4">
					<div class="relative w-full max-w-3xl bg-white rounded-3xl shadow-2xl overflow-hidden">
						<div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
							...
							<button type="button" onclick="document.getElementById('addMachineryModal').classList.add('hidden')"
								class="w-9 h-9 rounded-xl text-slate-400 hover:text-slate-600 hover:bg-slate-100 flex items-center justify-center transition">
								<i class="fa-solid fa-xmark"></i>
							</button>
						</div>
						<form action="{{ $storeRoute }}" method="POST" enctype="multipart/form-data" id="addMachineryForm">
							...
							<div class="p-6 grid grid-cols-1 sm:grid-cols-[220px_1fr] gap-6">
								<div>
									...
									<label for="image_path" class="cursor-pointer group block">
										<div id="imagePreviewWrapper"
											class="w-full aspect-square rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 group-hover:border-emerald-400 flex flex-col items-center justify-center overflow-hidden transition relative">
											<img id="imagePreview" src="" alt="Machinery preview"
											...
											<span id="imagePreviewLabel"
												class="text-[11px] font-semibold text-slate-400 group-hover:text-emerald-500 px-4 text-center">Click to
												upload image</span>
											<div id="imagePreviewOverlay"
												class="hidden absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 items-center justify-center transition">
								...
								<div>
									<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
										<div class="sm:col-span-2">
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Machinery Name <span
													class="text-red-500">*</span></label>
											<input type="text" name="machinery_name" placeholder="e.g. Kubota Tractor" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Model <span
													class="text-red-500">*</span></label>
											<input type="text" name="model" placeholder="e.g. L3901" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Serial Number <span
													class="text-red-500">*</span></label>
											<input type="text" name="serial_number" placeholder="e.g. TR-2026-001" required
												class="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-sm">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Rent / Hour (₱) <span
													class="text-red-500">*</span></label>
							...
							<div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-2">
								<button type="button" onclick="document.getElementById('addMachineryModal').classList.add('hidden')"
									class="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-600 text-xs font-semibold hover:bg-slate-100 transition">Cancel</button>
								<button type="submit"
									class="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition">Add
									Machinery</button>
							</div>
						</form>
					</div>
