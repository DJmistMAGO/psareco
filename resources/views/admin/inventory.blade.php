    ...
    openEdit(item) {
		...
		<main class="w-full min-w-0 p-4 sm:p-6 lg:p-8">
			...
			<x-page-header eyebrow="PSARECO Inventory" title="Inventory Management"
				...
				<x-slot:actions>
					<button type="button" onclick="document.getElementById('addProductModal').classList.remove('hidden')"
									...
									<tr class="hover:bg-slate-50/70 transition-colors">
										...
										<td class="px-5 py-4">
											<div class="flex flex-col gap-1 items-start">
												...
												@else
													<span
														class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-bold">
										...
										<td class="px-5 py-4">
											<div class="flex items-center justify-end gap-1">
												...
												<button type="button" @click="openView(@js($item))" title="View details"
													class="w-8 h-8 inline-flex items-center justify-center rounded-lg text-emerald-400 hover:text-emerald-600 hover:bg-emerald-50 transition">
												...
												<x-confirm-modal title="Archive Product" :message="'Archive ' . $item->name . '? This will move it to the archive — you can restore it later.'" confirmText="Archive"
													...
													method="DELETE">
													<button type="button" title="Archive product"
						...
						@foreach ($inventories as $item)
							...
							<div class="p-4">
								<div class="flex items-start justify-between gap-3">
									...
									@if ($isLowStock)
										<span
											class="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-50 text-red-600 text-[10px] font-bold">
											Low
										</span>
									@else
										<span
											class="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-bold">
											OK
										</span>
									@endif
								</div>
                                ...
                                <i class="fa-regular fa-pen-to-square text-xs"></i>
									...
									<x-confirm-modal title="Delete Product" :message="'Delete ' . $item->name . '? This will move it to trash — you can restore it later.'" confirmText="Delete"
										confirmClass="bg-red-600 hover:bg-red-700 text-white" icon="shield-alert" :action="route('inventory.deleteProduct', $item->id)" method="DELETE">
		...
		<div x-show="showView" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog">
			...
			<div class="relative min-h-screen flex items-center justify-center p-4">
				<div class="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden" x-show="showView"
					x-transition>
							...
							<div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
								<div class="flex items-center gap-3 min-w-0">
									<div class="w-10 h-10 shrink-0 rounded-xl flex items-center justify-center"
										:class="selected.type === 'Fertilizer' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'">
									...
									<div class="mb-6">
										...
										<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
											...
											<div>
												...
												<div
													class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 flex items-center gap-2">
													<i class="fa-solid text-xs"
														:class="selected.type === 'Fertilizer' ? 'fa-leaf text-emerald-600' : 'fa-bug text-amber-600'"></i>
												...
												<div class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200">
													<div class="flex items-baseline gap-1.5">
														<span class="text-sm font-bold"
															:class="Number(selected.quantity) <= Number(selected.reorder_level) ? 'text-red-600' : 'text-slate-700'"
															x-text="selected.quantity"></span>
											...
											<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
												<div class="flex items-center gap-3">
													<div class="w-9 h-9 rounded-xl flex items-center justify-center"
														:class="Number(selected.quantity) <= Number(selected.reorder_level) ? 'bg-red-50 text-red-600' :
														    'bg-emerald-50 text-emerald-600'">
														<i class="fa-solid"
															:class="Number(selected.quantity) <= Number(selected.reorder_level) ? 'fa-triangle-exclamation' :
															    'fa-circle-check'"></i>
													</div>
													<div>
		...
		<div x-show="showEdit" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true" role="dialog"
			x-data="{ editImagePreview: null }" @edit-opened.window="editImagePreview = null">
			<div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm" @click="showEdit = false"></div>

			<div class="relative min-h-screen flex items-center justify-center p-4">
				<div class="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl overflow-hidden" x-show="showEdit"
					x-transition>
								...
								<label for="edit_image_path" class="cursor-pointer group block">
									<div
										...
										<img x-show="editImagePreview || editForm.image_path"
											:src="editImagePreview ? editImagePreview : `/storage/${editForm.image_path}`" alt="Preview"
										...
										<div
											class="absolute inset-0 bg-slate-900/0 group-hover:bg-slate-900/40 flex items-center justify-center transition">
									...
									<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
										...
										<div>
											...
											<div class="relative">
												...
												<input type="number" name="price" x-model="editForm.price" min="0" step="0.01" required
													class="w-full pl-9 pr-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
											</div>
										</div>

										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Reorder Level</label>
											<input type="number" name="reorder_level" x-model="editForm.reorder_level" min="0" step="0.01"
												required
												class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
										</div>
									</div>
		...
		{{-- add product modal --}}
		<div id="addProductModal" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-labelledby="addProductTitle"
					...
					<form action="{{ route('inventory.addProduct') }}" method="POST" enctype="multipart/form-data"
								...
								<label for="image_path" class="cursor-pointer group block">
									<div id="imagePreviewWrapper"
										...
										<span id="imagePreviewLabel"
											class="text-[11px] font-semibold text-slate-400 group-hover:text-emerald-500 px-4 text-center">
											Click to upload image
										</span>
							...
							<div>
								<div class="mb-6">
									...
									<div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
										<div class="sm:col-span-2">
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Product Name</label>
											<input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Urea 46-0-0" required
										...
										<div class="sm:col-span-2">
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Description</label>
											<input type="text" name="description" placeholder="e.g. 50kg per bag/sack, 500ml per bottle"
												class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
										</div>
									</div>
								</div>
								...
								<div class="mb-6">
									...
									<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
										<div>
											...
											<input type="number" name="quantity" min="0" step="0.01" placeholder="0" required
												class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
										</div>
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Unit Price</label>
										...
										<div>
											<label class="block text-xs font-semibold text-slate-600 mb-1.5">Reorder Level</label>
											<input type="number" name="reorder_level" min="0" step="0.01" placeholder="10" required
												class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
										</div>
									</div>
								</div>
						...
						<div
							...
							<button type="submit" id="addProductSubmitBtn"
								class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition disabled:opacity-70 disabled:cursor-not-allowed">
