...
@section('content')
	...
	<main class="w-full min-w-0 p-4 sm:p-6 lg:p-8">
		<x-dashboard-header />
		<x-page-header eyebrow="Dashboard" title="Welcome Back {{ auth()->user()->name ?? 'User' }}!"
		...
		@role('admin|officer')
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
					...
					<div class="bg-white rounded-2xl shadow-sm border border-slate-100/80 p-5 flex flex-col min-h-[180px]">
						<div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
							...
							@if ($unreadNotifications > 0)
								<span
									class="inline-flex items-center px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 text-[10px] font-bold">
						...
						@php
							$notifications = auth()
							    ->user()
							    ->notifications->sortByDesc(function ($notification) {
							        return [$notification->read_at === null ? 1 : 0, $notification->created_at->timestamp];
							    })
							    ->take(4);
						@endphp
						...
						<div class="space-y-2">
							@forelse ($notifications as $notification)
								<a href="{{ route('notifications.redirect', $notification->id) }}"
									class="flex items-start gap-3 p-3 rounded-xl {{ $notification->read_at ? 'bg-slate-50' : 'bg-emerald-50 border border-emerald-100' }} hover:bg-emerald-50 transition-colors">
									<div
										class="w-8 h-8 rounded-lg {{ $notification->read_at ? 'bg-slate-100 text-slate-500' : 'bg-emerald-100 text-emerald-600' }} flex items-center justify-center shrink-0">
									...
									@if (!$notification->read_at)
										<span class="relative flex w-2 h-2 shrink-0 mt-2">
							...
							@empty
								<div class="flex items-center justify-center py-8">
									<div class="flex items-center space-x-2 text-slate-400 text-xs font-medium">
							...
							<a href="{{ route('notifications.index') }}"
								class="mt-3 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-slate-50 hover:bg-emerald-50 text-xs font-semibold text-slate-500 hover:text-emerald-700 transition-colors">
				...
				<div class="bg-white rounded-2xl shadow-sm border border-slate-100/80 p-5 flex flex-col min-h-[180px] lg:col-span-2">
					<div class="flex items-center space-x-2 pb-3 mb-4 border-b border-slate-100">
		...
		@role('farmer')
			<section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
				<div
					class="bg-white rounded-2xl p-4 shadow-sm border-t-4 border-emerald-500 flex items-center justify-between transition-all hover:-translate-y-0.5 hover:shadow-md">
					<div>
				...
				<div
					class="bg-white rounded-2xl p-4 shadow-sm border-t-4 border-amber-400 flex items-center justify-between transition-all hover:-translate-y-0.5 hover:shadow-md">
					<div>
						<p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">
							Pending Requests
						</p>
				...
				<div
					class="bg-white rounded-2xl p-4 shadow-sm border-t-4 border-sky-500 flex items-center justify-between transition-all hover:-translate-y-0.5 hover:shadow-md sm:col-span-2 lg:col-span-1">
					<div>
						<p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">
							Total Reservations
						</p>
			...
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
				<div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100/80 overflow-hidden flex flex-col">
					...
					<div class="w-full overflow-x-auto">
						<table class="w-full text-left border-collapse">
							<thead>
								<tr
									class="bg-slate-50/70 text-slate-500 text-[11px] uppercase tracking-wider font-semibold border-b border-slate-100">
								...
								@forelse ($myBookings as $booking)
									<tr class="hover:bg-slate-50/60 transition-colors">
										<td class="py-3.5 px-4 font-medium text-slate-800">
											<div class="flex items-center space-x-3">
												<div
													class="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center shrink-0 text-xs">
													<i class="fa-solid fa-tractor"></i>
												</div>
										...
										<td class="py-3.5 px-4 text-center whitespace-nowrap">
											...
											@endphp
											<span
												class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border {{ $statusClasses }}">
										...
										<td colspan="3" class="py-12 text-center text-slate-400">
											<div class="flex flex-col items-center">
												...
												<p class="text-[11px] text-slate-400 mt-0.5">
													Click "New Booking" to reserve farm machinery.
												</p>
											</div>
										</td>
									</tr>
				...
				<div
					class="bg-gradient-to-br from-emerald-800 to-teal-900 rounded-2xl shadow-sm p-6 text-white flex flex-col justify-between relative overflow-hidden">
					...
					<div>
						<span
							class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase bg-emerald-500/20 text-emerald-200 backdrop-blur-md mb-4 border border-emerald-400/20">
							Quick Reservation
						</span>

						<h4 class="text-xl font-bold mb-2">
							Need Equipment for Your Farm?
						</h4>
					...
					<a href="{{ route('farmers.index') }}"
						class="inline-flex items-center justify-center space-x-2 w-full py-3 px-4 bg-emerald-500 hover:bg-emerald-400 text-white font-bold text-xs rounded-xl shadow-lg transition-all text-center">
