<div x-show="mobileOpen" x-cloak x-transition:enter="transition-opacity ease-linear duration-200"
...
<aside @click.stop
	...
	<div>
		<div class="relative mb-6">
			...
			<button type="button" @click="sidebarOpen = !sidebarOpen" :class="sidebarOpen ? 'w-full' : 'w-10 mx-auto'"
			...
			<button type="button" @click="mobileOpen = false"
				class="lg:hidden absolute right-0 top-0 w-9 h-9 flex items-center justify-center rounded-lg text-slate-500 hover:bg-emerald-100 hover:text-emerald-800 transition"
		...
		<div class="flex flex-col items-center text-center mb-8">
			<div
				class="w-12 h-12 lg:w-14 lg:h-14 bg-white rounded-full flex items-center justify-center shadow-sm border border-emerald-100 mb-2 shrink-0 transition-all">
			...
			@php
				...
				if (auth()->user()->hasRole('admin')) {
				    ...
				    $menu = [
				        ...
				        [
				            'route' => 'farmers.myBookings',
				            'active' => 'farmers.myBookings',
	...
	<div class="space-y-3 pt-4 border-t border-slate-200/60">
		<div class="flex items-center space-x-3 p-1.5">
			<div
				class="w-8 h-8 rounded-full bg-[#276447] text-white font-bold flex items-center justify-center text-xs shadow-sm shrink-0">
		...
		<form method="POST" action="{{ route('logout') }}" @submit="mobileOpen = false">
			@csrf
			<button type="submit"
				class="w-full bg-[#fce8e6] hover:bg-[#f8d0cb] text-[#d9381e] font-medium py-2 px-3 rounded-xl flex items-center justify-center space-x-2 text-xs transition-all duration-200 active:scale-[0.98]"
				title="Logout">
